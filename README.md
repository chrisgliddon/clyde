# Clyde
Named after a certain character in Final Fantasy VI, this project is an experiment to see if we can build something fun for the SNES with 65816 Assembly langauge.

## Questions
1. Do we know enough to write a full RPG game in 65816 Assembly?
2. Can we write performant, modular code?
3. How do we avoid halllucinations?

## Documentation
- https://ersanio.gitbook.io/assembly-for-the-snes
- https://en.wikibooks.org/wiki/Super_NES_Programming/Initialization_Tutorial
- https://wiki.superfamicom.org/learning-65816-assembly

## Other Notes
- https://www.chibiakumas.com/6502/snes.php
- https://blog.wesleyac.com/posts/snes-dev-1-getting-started
- https://georgjz.github.io/snesaa01/

## Video Tutorials
- https://www.youtube.com/watch?v=iLXlnMc3pWw
- https://www.youtube.com/watch?v=kYLJLJkVfLk

## Tools
- https://snes.nesdev.org/wiki/Tools

## Interesting Projects
- An attempt at a SNES engine: https://github.com/Selicre/homebrew
- A WIP attempt to decompile Harvest Moon (SNES USA): https://github.com/dzik87/HarvestMoonSNES
- NBA Jam Assembly source code from 1992. Possibly the arcade version, not sure which flavor of Assembly it is. https://github.com/historicalsource/nbajam-complete/tree/main
