---
title: "es/PPU Registers"
reference_url: https://sneslab.net/wiki/es/PPU_Registers
categories:
  - "SNES_Hardware"
downloaded_at: 2026-02-14T17:32:45-08:00
...
UPI (también conocida como Unidad de Procesamiento de Imágenes o por su nombre en ingles PPU) es un co-procesador de SNES usado para enviar gestionar la información que es mostrada en la televisión para luego ser mostrada en pantalla. Este incluye varios registros que se pueden utilizar para crear efectos gráficos. **Varias partes de este articulo están basadas en el documento de Anomie "regs.txt"**

## Registros entre $2100~$21FF

### $2100: Brillo de Pantalla

- Puede ser leído o escrito en cualquier momento?: Si
- Puede ser leído o escrito durante el H-Blank?: Si
- Puede ser leído o escrito durante el V-Blank?: Si
- Puede ser leído o escrito durante el Force-Blank?: Si
- Puede ser utilizado para efectos gráficos?: Yes
- Register is readable for a value or effect (i.e. not open bus): Yes
- Tamaño del registro: 1 Byte

#### Formato:

x---bbbb

x = Activa el Force blank si es 1. bbbb = Nivel de brillo de la pantalla (F = Brillo normal, 0 = Totalmente oscuro).

Se debe notar que el Force Blank puede ser deshabilitado en una scanline intermedia. Sin embargo, esto puede resultar en errores graficos en esa scanline, debido a que el Buffer de renderizado interno no va a poder ser actualizado durante el Force Blank. La teoria actual es que el Backgroundse vera con errores por unos pocos tiles (dependiendo en cuan lejos la IPU hara operado) y los objetos se veran con errores en toda la scanline. Tambien, escribit en este registro en la primera linea del V-Blank (225 o 240, dependiendo del overscan) cuando esta en proceso de Force Blank, Causa que la OAM se resetee,

Este registro puede utilizarse con HDMA para hacer efectos de gradiente de brillo, también puede utilizarse para eliminar scanlines, esto produce un efecto de Widescreen que tiene como beneficio que durante el NMI se gasten menos ciclos, que pueden utilizarse para hacer mayor cantidad de envió de de información a la VRAM, videojuegos como Starfox utilizan esto para hacer envios masivos de graficos a la VRAM y simular graficos 3D. Para esto se puede realizar un HDMA similar a un gradiente de brillo pero en que el bit x este en 0 en la linea de de la parte superior de la pantalla y en la linea inferior de la pantalla.
